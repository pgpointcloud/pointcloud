...
PCPATCH *pc_patch_compress(const PCPATCH *patch, void *userdata)
{
    ...
    case PC_DIMENSIONAL: {
        if (patch_compression == PC_NONE) {
            ...
            PCPATCH_DIMENSIONAL *pcdu = pc_patch_dimensional_from_uncompressed(
            ...
            PCPATCH_DIMENSIONAL *pal =
                pc_patch_dimensional_from_uncompressed(pcu);
            PCPATCH_DIMENSIONAL *palc =
                pc_patch_dimensional_compress(pal, NULL);
    ...
    case PC_NONE: {
        if (patch_compression == PC_NONE) {
            ...
            PCPATCH_UNCOMPRESSED *pcu = pc_patch_uncompressed_from_dimensional(
    ...
    case PC_LAZPERF: {
        if (patch_compression == PC_NONE) {
            PCPATCH_LAZPERF *pgc = pc_patch_lazperf_from_uncompressed(
                (PCPATCH_UNCOMPRESSED *)patch);
            if (!pgc)
            ...
            PCPATCH_UNCOMPRESSED *pad = pc_patch_uncompressed_from_dimensional(
                (PCPATCH_DIMENSIONAL *)patch);
            PCPATCH_LAZPERF *pal =
...
PCPATCH *pc_patch_from_wkb(const PCSCHEMA *s, uint8_t *wkb, size_t wkbsize)
{
    ...
    */
    ...
    if (pcid != s->pcid) {
        pcerror("%s: wkb pcid (%d) not consistent with schema pcid (%d)",
                __func__, pcid, s->pcid);
    }
    ...
    default: {
        /* Don't get here */
        pcerror("%s: unknown compression '%d' requested", __func__,
                compression);
        return NULL;
    }
    }
...
uint8_t *pc_patch_to_wkb(const PCPATCH *patch, size_t *wkbsize)
{
    ...
    */
    switch (patch->type) {
    case PC_NONE: {
        return pc_patch_uncompressed_to_wkb((PCPATCH_UNCOMPRESSED *)patch,
                                            wkbsize);
    }
    case PC_DIMENSIONAL: {
        return pc_patch_dimensional_to_wkb((PCPATCH_DIMENSIONAL *)patch,
                                           wkbsize);
    }
    case PC_LAZPERF: {
                             ...
                             double def)
{
    ...
    for (j = 0; j < new_schema->ndims; j++) {
        ...
        PCDIMENSION *odim =
            pc_schema_get_dimension_by_name(old_schema, ndim->name);
