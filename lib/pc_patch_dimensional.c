                              ...
                              PCDIMSTATS *pds_in)
{
    ...
    for (i = 0; i < ndims; i++) {
        pdl_compressed->bytes[i] = pc_bytes_encode(
            pdl->bytes[i], pds->stats[i].recommended_compression);
    }
                                       ...
                                       const uint8_t *wkb, size_t wkbsize)
{
    ...
    */
    ...
    if (wkb_get_compression(wkb) != PC_DIMENSIONAL) {
        pcerror("%s: call with wkb that is not dimensionally compressed",
                __func__);
        return NULL;
    }
...
PCPATCH_DIMENSIONAL *pc_patch_dimensional_from_pointlist(const PCPOINTLIST *pdl)
{
    ...
    if (!patch)
        return NULL;
    PCPATCH_DIMENSIONAL *dimpatch =
        pc_patch_dimensional_from_uncompressed(patch);
    pc_patch_free((PCPATCH *)patch);
    return dimpatch;
}
