...
int main(int argc, char *argv[]) {
  ...
  if (argc <= 1) {
     ...
     * routinely called by users." However, there didn't seem to be any other
     * way to get tests by name, so we're calling them. */
    ...
    for (index = 1; index < argc; index++) {
      ...
      if (suite_to_run == NULL) {
        printf("\n'%s' does not appear to be either a suite name or a test "
               "name.\n\n",
               suite_name);
      } else {
        if (test_name != NULL) {
           ...
           * This one doesn't output anything for success, so we have to do it
           * manually. */
          errCode = CU_run_test(suite_to_run, test_to_run);
          if (errCode != CUE_SUCCESS) {
            printf("    Error attempting to run tests: %d.  See CUError.h for "
                   "error code list.\n",
                   errCode);
          } else {
            ...
            printf(
                ...
                (0 == num_failed ? "PASSED" : "FAILED"), (num_run - num_failed),
                num_failed, num_run);
          }
        } else {
    ...
    /* Presumably if the CU_basic_run_[test|suite] functions worked, we wouldn't
     * have to do this. */
