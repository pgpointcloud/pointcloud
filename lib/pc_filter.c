                             ...
                             const PCBITMAP *map)
{
    ...
    PCPATCH_UNCOMPRESSED *fpu =
        pc_patch_uncompressed_make(pu->schema, map->nset);
    uint8_t *buf = pu->data;
                                             ...
                                             double val2)
{
    ...
    double unscaled1 =
        pc_value_unscale_unoffset(val1, pdl->schema->dims[dimnum]);
    double unscaled2 =
        pc_value_unscale_unoffset(val2, pdl->schema->dims[dimnum]);
                         ...
                         PC_FILTERTYPE filter, double val1, double val2)
{
    ...
    switch (pa->type) {
    case PC_NONE: {
        PCBITMAP *map = pc_patch_uncompressed_bitmap(
            (PCPATCH_UNCOMPRESSED *)pa, dimnum, filter, val1, val2);
