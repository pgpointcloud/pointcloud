...
static void pc_schema_calculate_byteoffsets(PCSCHEMA *pcs)
{
    ...
    for (i = 0; i < pcs->ndims; i++) {
        if (pcs->dims[i]) {
            ...
            pcs->dims[i]->size =
                pc_interpretation_size(pcs->dims[i]->interpretation);
...
PCSCHEMA *pc_schema_from_xml(const char *xml_str)
{
    ...
    static xmlChar *xpath_str =
        (xmlChar *)("/pc:PointCloudSchema/pc:dimension");
        ...
        for (i = 0; i < ndims; i++) {
            ...
            if (nodes->nodeTab[i]->type == XML_ELEMENT_NODE) {
                ...
                for (child = cur->children; child; child = child->next) {
                    if (child->type == XML_ELEMENT_NODE &&
                        ...
                        if (strcmp(name, "name") == 0)
                            ...
                            d->interpretation =
                                pc_interpretation_number(content);
                ...
                if (d->position >= ndims) {
                    ...
                    pcwarn(
                        "schema dimension at position \"%d\" is declared twice",
                        d->position + 1, ndims);
                    pc_dimension_free(d);
                    goto cleanup;
                }
