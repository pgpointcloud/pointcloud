...
>
> For dimensions that are in the "new" schema but not in the old "schema" the value `0.0` is used.

**PC_SetPCId(p pcpatch, pcid int4, defaults float8)** returns **pcpatch**

> Same as `PC_SetPCId(p pcpatch, pcid int4)` but using `defaults` as the default value instead of `0.0`.
